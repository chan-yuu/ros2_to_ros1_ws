...
#include <ros/console.h> // 确保可以使用ROS_INFO宏
...
sensor_msgs::PointCloud2 MapLoader::TransformMap(sensor_msgs::PointCloud2 & in){
    ...
    // 打印变换后点的数量
    ROS_INFO("Transformed point cloud size: %zu", transformed_pc_ptr->size());

    // 应用Voxel Grid滤波
    pcl::PointCloud<pcl::PointXYZ>::Ptr filtered_pc_ptr(new pcl::PointCloud<pcl::PointXYZ>());
    pcl::VoxelGrid<pcl::PointXYZ> voxel_filter;
    voxel_filter.setInputCloud(transformed_pc_ptr);
    voxel_filter.setLeafSize(0.1f, 0.1f, 0.1f); // 设置体素大小，根据需求调整
    voxel_filter.filter(*filtered_pc_ptr);

    // 打印滤波后点的数量
    ROS_INFO("Filtered point cloud size: %zu", filtered_pc_ptr->size());

    SaveMap(filtered_pc_ptr);
    
    sensor_msgs::PointCloud2 output_msg;
    pcl::toROSMsg(*filtered_pc_ptr, output_msg);
    return output_msg;
}
