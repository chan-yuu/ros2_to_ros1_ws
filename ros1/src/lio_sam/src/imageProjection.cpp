...
public:
    ...
    bool deskewInfo()
    {
        ...
        // Check if IMU data is available, but don't require it for processing
        bool imuDataAvailable = true;
        if (imuQueue.empty() || imuQueue.front().header.stamp.toSec() > timeScanCur || imuQueue.back().header.stamp.toSec() < timeScanEnd)
        {
            ROS_DEBUG("IMU data not available, proceeding without IMU deskewing...");
        ...
        {
            // Set IMU unavailable flag
            cloudInfo.imuAvailable = false;
            // Initialize IMU orientation to zero
    ...
    void imuDeskewInfo()
    {
        ...
        for (int i = 0; i < (int)imuQueue.size(); ++i)
        {
            ...
            if (imuPointerCur >= queueLength - 1) {
                ROS_WARN("IMU queue buffer overflow, skipping remaining IMU data");
                break;
            }
    ...
    void findRotation(double pointTime, float *rotXCur, float *rotYCur, float *rotZCur)
    {
        *rotXCur = 0; *rotYCur = 0; *rotZCur = 0;
        ...
        // Add boundary check for imuPointerCur
        if (imuPointerCur <= 0) {
            return;
        }
        ...
        if (imuPointerFront >= queueLength) {
            imuPointerFront = queueLength - 1;
        }
    ...
    {
        // If deskew is disabled, return original point
        if (deskewFlag == -1)
            return *point;

        // If IMU is not available, only use odometry for deskewing if available
        if (cloudInfo.imuAvailable == false)
        {
            // Try to use odometry for deskewing if available
            if (cloudInfo.odomAvailable == true && odomDeskewFlag == true)
            {
                ...
                PointType newPoint;
                newPoint.x = point->x + posXCur;
                newPoint.y = point->y + posYCur;
                newPoint.z = point->z + posZCur;
                newPoint.intensity = point->intensity;
                return newPoint;
            }
            else
            {
                // No deskewing possible, return original point
                return *point;
            }
        }
    ...
    for (int i = 0; i < cloudSize; ++i)
    {
        ...
        if (has_ring == true)
            thisPoint = deskewPoint(&thisPoint, laserCloudIn->points[i].time);
                ...
                // Add safety check for orientationDiff to prevent division by zero
                float relTime = 0.0f;
                if (fabs(cloudInfo.orientationDiff) > 1e-6) {
                    relTime = (ori - cloudInfo.startOrientation) / cloudInfo.orientationDiff;
                    // Clamp relTime to valid range [0, 1]
                    relTime = std::max(0.0f, std::min(1.0f, relTime));
                }
                // 激光雷达10Hz，周期0.1
    ...
    void cloudExtraction()
    {
        ...
        for (int i = 0; i < N_SCAN; ++i)
        {
            ...
            for (int j = 0; j < Horizon_SCAN; ++j)
            {
                ...
                {
                    // Add boundary check to prevent buffer overflow
                    if (count >= N_SCAN * Horizon_SCAN) {
                        ROS_WARN("Cloud extraction buffer overflow, stopping extraction");
                        break;
                    }
                    ...
                    // Add boundary check for fullCloud access
                    int index = j + i * Horizon_SCAN;
                    if (index >= 0 && index < N_SCAN * Horizon_SCAN) {
