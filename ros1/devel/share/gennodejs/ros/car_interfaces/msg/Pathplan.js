...
class Pathplan {
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    obj.points.forEach((val) => {
      bufferOffset = Pointplan.serialize(val, buffer, bufferOffset);
    });
    return bufferOffset;
  }
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    for (let i = 0; i < len; ++i) {
      data.points[i] = Pointplan.deserialize(buffer, bufferOffset)
    }
    return data;
  }
  ...
  static Resolve(msg) {
    ...
    if (msg.points !== undefined) {
      ...
      for (let i = 0; i < resolved.points.length; ++i) {
        resolved.points[i] = Pointplan.Resolve(msg.points[i]);
      }
    }
    else {
      resolved.points = []
